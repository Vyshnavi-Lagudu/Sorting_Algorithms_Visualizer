...
async function merge(ele, low, mid, high){
    ...
    // copying elements before mid into first array
    for(let i = 0; i < n1; i++){
        await wait(delay);
    ...
    // copying elements after mid into second array
    for(let i = 0; i < n2; i++){
        await wait(delay);
    ...
    while(i < n1 && j < n2){
        ...
        // changing color of the elements that are being compared for merging       
        if(parseInt(left[i]) <= parseInt(right[j])){
            console.log('In merge while loop if');
            // changing color of sorted elements to green and others to lightgreen
            if((n1 + n2) === ele.length){
    ...
    while(i < n1){
        await wait(delay);
        console.log("If second array got completed first");
          // changing color of sorted elements to green and others to lightgreen
        if((n1 + n2) === ele.length){
            ele[k].style.background = 'green';
        }
    ...
    while(j < n2){
        await wait(delay);
        console.log("If first array got completed first");
          // changing color of sorted elements to green and others to lightgreen
        if((n1 + n2) === ele.length){
            ele[k].style.background = 'green';
        }
